...
async function runRuNetherlandsLaptopTest() {
  console.log('====================================================');
  console.log('🇷🇺 ➔ 🇳🇱 ТЕСТ ДВОЙНОГО VPN (РОССИЯ ➔ НИДЕРЛАНДЫ) НА НОУТБУКЕ');
  console.log('====================================================');

  try {
    const controlConn = await createSshClient();

    console.log('\n1. Передача обновленных файлов и ruservjson.txt через SFTP...');
    await new Promise((resolve, reject) => {
      controlConn.sftp(async (err, sftp) => {
        if (err) return reject(err);
        try {
          const files = ['server.js', 'auto-vpn-tester.js', 'serverjson.txt', 'ruservjson.txt', 'public/app.js', 'public/index.html', 'public/style.css'];
    ...
    const ruRaw = fs.readFileSync('ruservjson.txt', 'utf8');
    ...
    const ruVless = ruJson.outbounds.find(o => o.protocol === 'vless');
    const netherlandsVless = netherlandsJson.outbounds.find(o => o.protocol === 'vless');

    const relayNode = {
      name: '🇷🇺 - Россия (Реле)',
      protocol: 'vless',
      host: ruVless.settings.vnext[0].address,
      port: ruVless.settings.vnext[0].port,
      uuid: ruVless.settings.vnext[0].users[0].id,
      flow: ruVless.settings.vnext[0].users[0].flow,
      security: ruVless.streamSettings.security,
      sni: ruVless.streamSettings.realitySettings.serverName,
      pbk: ruVless.streamSettings.realitySettings.publicKey,
      sid: ruVless.streamSettings.realitySettings.shortId,
      fingerprint: ruVless.streamSettings.realitySettings.fingerprint
    };
    ...
    console.log('\n4. Запуск ДВОЙНОГО VPN (Россия ➔ Нидерланды)...');
    ...
    console.log('\n📊 РЕЗУЛЬТАТ IP ДВОЙНОГО VPN (Россия ➔ Нидерланды):');
